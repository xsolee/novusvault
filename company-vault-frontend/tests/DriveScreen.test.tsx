...
describe('DriveScreen (starts connected)', () => {
  it('shows the connected account, folder, and sync stats', async () => {
    ...
    expect(screen.getByText('Sync Documents')).toBeTruthy();
  ...
  it('disconnects Google Drive after confirming the dialog', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Disconnect')).toBeTruthy(), { timeout: 5000 });
    ...
    await fireEvent.press(screen.getAllByText('Disconnect')[1]);
